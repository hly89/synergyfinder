---
title: "Sample Script - Synergy Bootstrapped"
author: "P R Barber"
date: "07/01/2022"
...
Bayesian Bootstrap from the replicates to get better error bars.
...
dataimagefilename = "MultiDrugSynergy_BayesBootstrapped.Rdata"

#load(dataimagefilename)

methods <- c("ZIP")    # "ZIP", "Bliss", "HSA" and "Loewe"

#### Can EDIT THIS for more repeats, usually 1000 ####
# Bootstraps
nRepeats = 100
```

Number of bootstrap repeats: `r nRepeats`
...
CalculateSynergySummaryScore <- function(data, method = "ZIP", plot = FALSE){
    ...
    if(!is.null(synergy.score)) {
      ...
      if(plot)
        PlotSynergy2(synergy.score, type = "all") 
...
Using the R package "bayesboot".

Generate several draws from the distributions for every well of every condition given the replicates.

```{r Bootstrapping, warning=F, message=F, eval=T}

# Create "draws" tables that has nRepeats cols and a row for every well
# Can later use each col as a Response col

for(table in all_tables){
  x_table <- get(paste0(table, "_cast_table"))
  x_draws <- data.frame()
  replicates_cols <- grep("[1-99]", colnames(x_table), value = T)
  for(j in 1:nrow(x_table)){    # for all wells
    data <- t(as.matrix(x_table[j ,colnames(x_table) %in% replicates_cols]))
    draws <- t(bayesboot(data, R = nRepeats, statistic = weighted.mean, na.rm = T))
    x_draws <- rbind(x_draws, draws)
  }
  assign(paste0(table, "_draws"), x_draws)    
}

save.image(file = dataimagefilename)
```

For each draw of well values, generate synergy scores.

```{r, synergy, warning=F, message=F, eval=T}

for(table in all_tables){
  x_table <- get(paste0(table, "_cast_table"))
  x_draws <- get(paste0(table, "_draws"))

  for(method in methods){
  
    scores <- vector()

    for(i in 1:dim(x_draws)[2]){
    
      x_table$Response <- x_draws[,i]

      mat <- ReshapeData(x_table, data.type = "viability")
      
      s <- CalculateSynergySummaryScore(mat, method = method)
...
#  n=1
  ...
  results_table <- data.frame(all_tables, orig_scores, mean_scores, median_scores, 
