#' Visualize the drug combination dose-response data (for knitr)
#'
...
PlotDoseResponse2 <- function (data, pair.index = NULL, Emin = NA, 
    Emax = NA, ...) 
{
    if (!is.list(data)) {
...
#            "\n BlockID:", drug.pairs$blockIDs[i], sep = " ")
        ...
        dose.response.p <- ggplot(data.plot, aes_string(x = "x", 
            ...
            cunit.text, sep = " ")) + ylab(paste(drug.row, runit.text, 
            sep = " "))
...
#        layout(matrix(c(1, 3, 2, 3), 2, 2, byrow = TRUE))
 ...
 #       suppressWarnings(par(mgp = c(3, 0.5, 0)))
        suppressWarnings(par(mgp = c(1.5, 0.5, 0), mar=c(2.6, 2.6, 0.5, 0.5)))
        x.lab <- paste(drug.row, runit.text, sep = " ")
        plot(single.fitted$drug.row.model, xlab = x.lab, ylab = "Inhibition (%)", 
            type = "obs", col = "red", cex = 1, pch = 16, ...)
        plot(single.fitted$drug.row.model, xlab = x.lab, ylab = "Inhibition (%)", 
            type = "none", cex = 0.5, add = TRUE, lwd = 2)
#        title(paste("Dose-response:", drug.row, 
#            "in Block", drug.pairs$blockIDs[i]), cex.main = 1)
#        title(drug.row, cex.main = 1)
        x.lab <- paste(drug.col, cunit.text, sep = " ")
        plot(single.fitted$drug.col.model, xlab = x.lab, ylab = "Inhibition (%)", 
            type = "obs", col = "red", cex = 1, pch = 16, ...)
        plot(single.fitted$drug.col.model, xlab = x.lab, ylab = "Inhibition (%)", 
            type = "none", cex = 0.5, add = TRUE, lwd = 2)
#        title(paste("Dose-response:", drug.col, 
#            "in Block", drug.pairs$blockIDs[i]), cex.main = 1)
#        title(drug.col, cex.main = 1)
        plot.new()
        print(dose.response.p, vp = viewport(height = unit(1, 
            "npc"), width = unit(0.75, "npc"), just = c("left", 
            "top"), y = 1, x = 0.25))
